...
int main()
{
    ...
    while (std::getline(std::cin, line)) {
        if (!line.empty()) {
            rawTrees.push_back(line);
        }
    }
    int ans = INT_MIN;
    for (int i = 0; i < rawTrees.size(); ++i) {
        for (int j = 0; j < rawTrees.size(); ++j) {
            if (i == j) {
                continue;
            }
            std::stringstream ss(rawTrees[i]);
            Node* left = readTree(ss);
            ss = std::stringstream(rawTrees[j]);
            Node* right = readTree(ss);
            ans = std::max(ans, add(left, right));
            delete left;
            delete right;
        }
    }
