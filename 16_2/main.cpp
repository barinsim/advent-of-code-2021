...
ll traverse(Packet* root, Func func)
{
    if (std::holds_alternative<Operator>(root->data)) {
        std::vector<ll> results;
        auto& op = std::get<Operator>(root->data);
        for (auto& p : op.packets) {
            results.push_back(traverse(p.get(), func));
        }
        return func(results, root->header.typeId);
    } else {
        return std::get<Literal>(root->data).num;
    }
}
...
int main()
{
    ...
    ll res = traverse(&root, [](std::vector<ll>& nums, int op) -> ll {
        switch (op) {
            case 0 : return std::accumulate(nums.begin(), nums.end(), 0LL);
            case 1 : return std::accumulate(nums.begin(), nums.end(), 1LL, std::multiplies<ll>());
            case 2 : return *std::min_element(nums.begin(), nums.end());
            case 3 : return *std::max_element(nums.begin(), nums.end());
            case 5 : return nums[0] > nums[1];
            case 6 : return nums[0] < nums[1];
            case 7 : return nums[0] == nums[1];
        }
        return -1;
    });
